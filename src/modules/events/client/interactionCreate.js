...
} = require("../../utils/essentials/modalHandler");
const {
  showRegisterClanchatModal,
} = require("../../modals/registerCCWebhookModal");
const modalIds = require("../../../config/customModalIds");
const db = require("../../utils/essentials/dbUtils");
const {
  sanitizeInteraction,
} = require("../../utils/essentials/security/discordSanitizer");
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isCommand()) {
      ...
      if (modalIds.includes(interaction.customId)) {
        ...
        await db.runQuery(
          "INSERT INTO modal_tracking (modal_key, registered_by) VALUES (?, ?)",
          [interaction.customId, interaction.user.id]
        );
      ...
      if (
        interaction.customId === "vote_dropdown" &&
        client.competitionService
      ) {
